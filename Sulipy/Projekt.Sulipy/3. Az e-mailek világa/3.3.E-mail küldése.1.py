...
az e-mail külömböző paramétereit (küldő, címzett, tárgy, üzenet szövege)!
'''
...
def gmail_create_and_send_draft_with_attachment(service, cimzett, targy, uzenet, csatolmany):
    """Create and send a draft email with attachment."""
    try:
        mime_message = EmailMessage()

        # headers
        mime_message["To"] = cimzett
        mime_message["From"] = "zsolt.gondocs@gmail.com"
        mime_message["Subject"] = targy

        # text
        mime_message.set_content(uzenet)

        # attachment
        attachment_filename = csatolmany
...
if __name__ == "__main__":
    cimzett = input('A címzett e-mail címe: ')
    targy = input('Tárgy: ')
    uzenet = input('Üzenet: ')
    csatolmany = input('Csatolmány link: ')
    service = get_service()
    gmail_create_and_send_draft_with_attachment(service, cimzett, targy, uzenet, csatolmany)
