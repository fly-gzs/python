...
with open('./adatok/A_csoport.txt', 'r', encoding='utf-8') as forrasfajl:
    forrasfajl.readline()
    for sor in forrasfajl:
        adatok = sor.strip().split(',')
        csoport_adatok = {
            'Németország': {'meccs':int(adatok[1]), 'gyozelem':int(adatok[2]), 'dontetlen':int(adatok[3]), 'vereseg':int(adatok[4]), 'rugott_gol':int(adatok[5]), 'kapott_gol':int(adatok[6]), 'pont':int(adatok[7])},
            'Skócia': {'meccs':int(adatok[1]), 'gyozelem':int(adatok[2]), 'dontetlen':int(adatok[3]), 'vereseg':int(adatok[4]), 'rugott_gol':int(adatok[5]), 'kapott_gol':int(adatok[6]), 'pont':int(adatok[7])},
            'Magyarország':{'meccs':int(adatok[1]), 'gyozelem':int(adatok[2]), 'dontetlen':int(adatok[3]), 'vereseg':int(adatok[4]), 'rugott_gol':int(adatok[5]), 'kapott_gol':int(adatok[6]), 'pont':int(adatok[7])},
            'Svájc': {'meccs':int(adatok[1]), 'gyozelem':int(adatok[2]), 'dontetlen':int(adatok[3]), 'vereseg':int(adatok[4]), 'rugott_gol':int(adatok[5]), 'kapott_gol':int(adatok[6]), 'pont':int(adatok[7])}
            }


with open('./adatok/A_meccsek.txt', 'r', encoding='utf-8') as forrasfajl:
    forrasfajl.readline()
    for sor in forrasfajl:
        adatok = sor.strip().split(', ')
...
with open('./adatok/A_csoport_masolat.txt', 'w', encoding='utf-8') as celfajl:
    print('', file=celfajl)
    for csapat in A_csoport:
...
with open('./adatok/A_csoport_masolat.txt', 'r', encoding='utf-8') as forrasfajl:
    forrasfajl.readline()
    for sor in forrasfajl:
        adatok = sor.strip().split(',')
...
print('\n--- Az "A" csoport pontok és gólkülönbség szerint: ---')
for csapat in pontokszerint:
    print(csapat[0], ': ', ', '.join(map(str, csapat[1:])), sep='')

with open('./adatok/A_csoport_állás.txt', 'w', encoding='utf-8') as celfajl:
    print('', file=celfajl)
    for csapat in pontokszerint:
