...
if token:
    ...
    update_url = f"http://192.168.2.150/api/v2.0.0/extension/list?token={token}"
    update_headers = {
        "Content-Type": "application/json; charset=utf-8"
    }
    ...
    if update_response.json()['status'] == 'Success':
        print("A melléklista sikeresen lekérve!")
        pprint(update_response.json())
    else:
        print(f"Hiba történt a melléklista készítésekor!")
